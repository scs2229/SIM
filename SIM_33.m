% SIM_33 is a zero-dimensional model which calculates ice and
...
% tracked as in Korolev and Mazin 2003 Appendix B. 
% Code Development: Sylvia Sullivan 2016 - 2017, KIT, GIT, CU
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% inputs:
...
function [Nice,soln,INPt,tauNUC] = SIM_33(etabr,etaagg,etaRSg,etaRSG,etaCOA,...
    etaSH,duration,step,INPstop)  
    ...
    mice    = rhoi/rhoa*Ni*4/3*pi*ri0^3;
    mliq    = rhow/rhoa*Nd*4/3*pi*rw0^3;
    init(2) = mliq;          %/(1 + mice); 
    init(3) = mice;          %/(1 + mliq);
    ...
    for tt = 2:length(timepts) - 1
        ...
        if (INPt.y(tt) >= INPstop*imm && counter3 == 1)
            tauNUC   = timepts(tt);
            counter3 = 2;
        end
        
        rhodep  = depDens(T0, P0, sw);
        options = ddeset('RelTol',1e-3,'AbsTol',5e-3);    
        ...
        % halt simulation if any large hydrometeor numbers become negative
        if (NG < 0 || NR < 0) 
            break 
        end
        ...
        mice = rhoi/rhoa*Ni*4/3*pi*ri0^3 + rhodep/rhoa*(Ng*4/3*pi*ag0^(iG(T0)+2) + ...
            NG*4/3*pi*aG0^(iG(T0)+2));
        mliq = rhow/rhoa*4/3*pi*(Nd*rw0^3 + Nr*rr0^3 + NR*rR0^3);
        init(3) = mice;          %/(1 + mliq); 
        init(2) = mliq;          %/(1 + mice);
    ...
    function dy = microphysics(t, y, Z)
        ...
        if (set3 == false)
            dy(1) = nucl(y(7))*imm*heaviside(tauNUC-t) + ...
                etabr*Kcoll*Ncoll*y(2)*y(3) + etaRSg*KRSg*NRS*y(2)*y(6) + ...
                etaRSG*KRSG*NRS*y(3)*y(6) + etaSH*pSH*pFR*NSH*y(6) - ...
        ...
        cont.SH = [cont.SH etaSH*NSH*pSH*y(6)];
