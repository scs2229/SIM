% SIM_33prob is a zero-dimensional model which calculates ice and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% outputs: 
% probFRZ  = freezing probability for a large droplet
...
function [probFRZ,Nice,cont,INPt,tauNUC] = SIM_33prob(etabr,etaagg,etaRSg,...
    etaRSG,etaCOA,etaSH,duration,step,INPstop)
    ...
    for tt = 2:length(timepts) - 1
        ...
        % store the probability of a large droplet freezing calculate in
        % the microphysics
        probFRZ(tt+1) = pFR;
    ...
    Nice    = Nice(:,1:indx);
    indx    = find(probFRZ ~= 0, 1, 'last');
    probFRZ = probFRZ(:,1:indx);
