% SIM_35thermo is a zero-dimensional model which calculates ice and
...
%            default of 0.2
% Fshatter = coefficient for fragments formed upon droplet shattering;
%            default of 2.5 x 10^(-11)
%            BE CAREFUL! if coll == true, this is the number of fragments
...
% sig      = 1 to use D^4 Nsh function, 2 to use D^3, 3 to use sigmoid
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% outputs: 
...
function enhancement = SIM_35thermo(etabr,etaagg,etaRSg,etaRSG,...
    etaCOA,etaSH,duration,step,INPstop,uz,Tstart,tau,Fshatter,coll,sig)
    ...
    taui   = tau(1);   % crystal growth time
    taug   = tau(2);   % graupel growth time
    tauf   = tau(3);   % fallout time 
    taud   = tau(4);   % droplet growth time
    taur   = tau(5);   % larger droplet growth time
    tauR   = tau(6);   % fallout time
    ...
    function frag = Neject(T)
        gamma = 5; alpha = 1.2;
        frag = 280*(T - 252)^(alpha)*exp(-(T - 252)/gamma);
        frag(frag < 0) = 0; %10
    end
    ...
    % determine the number of ice fragments ejected upon shattering of a
    % frozen droplet with 3rd order dependence on droplet size
    % D [=] m
    function frag = Nshatter3(D)
        D = D/10^(-6);
        frag = Fshatter*D^3;
    end
    ...
    function frag = NshatterSIG(D)
        if numel(Fshatter) ~= 3
            error('Fshatter must be a three element array');
        end
        aa = Fshatter(1);   % 10 / 9 by default
