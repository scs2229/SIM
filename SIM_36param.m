% SIM_36param is a zero-dimensional model which calculates ice and
...
% tracked as in Korolev and Mazin 2003 Appendix B. Secondary production
% tendency parameters should be given as inputs here.
...
% pmax     = maximum probability that a frozen droplet shatters; 
%            default of 0.2
% Fshatter = two-element vector with coefficient for fragments formed 
%            upon droplet shattering (default = 2.5 x 10^(-11)) and order
%            with respect to diameter (default = 4)
%            BE CAREFUL! if sig == true, two-element vector with the beta
%            and gamma parameters for the sigmoid (-0.016 and 500 by
%            default)
%            BE CAREFUL! if coll == true, the first element is simply the
%            number of fragments formed the second element is ignored
%            if sig == 3, this needs to be a 3-element array for sigmoid
% Fbr      = coefficient for fragments formed upon breakup;
%            default of 280
% Tminbr   = minimum temperature at which breakup occurs;
%            default of 252
% FRS      = coefficient for fragments formed upon rime splintering;
%            default of 3 x 10^8
% factor   = nucleation reduction factor at warmer subzero temperatures;
%            default of 1
% coll     = boolean to determine droplet shattering formulation; if true,
%            collisional shattering; if false, not collisional
% sig      = boolean to calculate fragments generated with a sigmoid not a
%            polynomial in diameter as in Lawson
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% outputs: 
% enhancement = total ice hydrometeor number, i.e., Ni (1) + Ng (2) + NG (3)
% versus cumulative number of nucleated INP, both from the end of the 
% simulation 

function enhancement = SIM_36param(etabr,etaagg,etaRSg,etaRSG,...
    etaCOA,etaSH,duration,step,INPstop,pmax,Fshatter,Fbr,Tminbr,FRS,factor,...
    coll,sig)
    ...
    uz   = 2;       % updraft velocity (assuming single value for now) [m s-1]
    ...
    % these can also be solved for using the separate growth.m and 
    % crystalGrowth.m functions
    taui   = 7.5*60;   % crystal growth time
    taug   = 20*60;    % graupel growth time
    tauf   = 17.5*60;  % fallout time 
    taud   = 5*60;     % droplet growth time
    taur   = 15*60;    % larger droplet growth time
    tauR   = 25*60;    % fallout time
    ...
    for tt = 2:length(timepts) - 1
        ...
        rhodep  = depDens(T0, P0, sw);
        options = ddeset('RelTol',1e-3,'AbsTol',1e-3); 
    ...
    Nice   = Nice(:,1:indx);
    enhancement = Nice(end)/INPt.y(end);
    ...
    function dy = microphysics(t, y, Z)
        ...
        % calculate the sweep-out kernal for droplet shattering if necessary
        if (coll == true)
            KSH = pi*(ri0^2 + rR0^2)*vtR;
        end
        ...
        if (set3 == false)
            if (coll == false)
                ...
                dy(1) = nucl(y(7))*imm*heaviside(tauNUC-t) + ...
                    ...
                    etaRSg*KRSg*NRS*y(2)*y(6) + ...
                    etaRSG*KRSG*NRS*y(3)*y(6) + ...
                    etaSH*KSH*pSH*(NSH - 1)*y(1)*y(6) - ...
                    nucl(ylag1(7))*imm*heaviside(t-taui)*heaviside(tauNUC-t+taui) - ...
                    etabr*Kcoll*Ncoll*ylag1(2)*ylag1(3) - ...
                    etaRSg*KRSg*NRS*ylag1(2)*ylag1(6) - ...
                    etaRSG*KRSG*NRS*ylag1(3)*ylag1(6) - ...
                    etaSH*KSH*pSH*NSH*ylag1(1)*ylag1(6) - ...
                    etaagg*Kagg*y(1)*y(2);
            end
        else
            dy(1) = 0;
        ...
        if (set4 == false)
            if (coll == false)
                ...
                dy(2) = nucl(ylag1(7))*imm*heaviside(t-taui)*heaviside(tauNUC-t+taui) + ... 
                    ...
                    etaRSg*KRSg*NRS*ylag2(2)*ylag2(6) - ...    
                    etaRSG*KRSG*NRS*ylag2(3)*ylag2(6) - ...
                    etaSH*KSH*pSH*NSH*ylag2(1)*ylag2(6) - ...
                    etaagg*Kagg*y(1)*y(2); 
            end
        else
            dy(2) = 0;
        ...
        if (coll == false)
            ...
            dy(3) = nucl(ylag2(7))*imm*heaviside(t-taui-taug)*heaviside(tauNUC-t+taui+taug) + ... 
                ...
                etaRSg*KRSg*NRS*ylag3(2)*ylag3(6) - ...
                etaRSG*KRSG*NRS*ylag3(3)*ylag3(6) - ...
                etaSH*KSH*pSH*NSH*ylag3(1)*ylag3(6);   
        end
        ...
        if (coll == false)
            ...
            cont.SH = [cont.SH etaSH*KSH*(NSH - 1)*y(1)*y(6)];
        end
    ...
    function frag = Neject(T)
        gamma = 5; alpha = 1.2;
        frag = Fbr*(T - Tminbr)^(alpha)*exp(-(T - Tminbr)/gamma);
        frag(frag < 0) = 0; %10
    end
    ...
    function frag = Nshatter(D)
        D = D/10^(-6);
        frag = Fshatter(1)*D^(Fshatter(2));
    end

    % determine the number of ice fragments ejected upon shattering of a
    % frozen droplet as a function of droplet size (sigmoid)
    function frag = NshatterSIG(D)
        aa = 10; 
        bb = Fshatter(1);   % -0.016 / -0.015 by default
        cc = Fshatter(2);   % 500 / 400 by default
        frag = aa/(1 + exp(bb*(D - cc)));
    end
    ...
    function p = pShatter(T)
        if (T < 266)
            mu  = 258; sigma = 5;
            max = pmax; % normalize to a max probability 
    ...
    % probability of a liquid droplet freezing, Bigg 1953, T [=] K
   ...
   % droplet activation rate based on the Twomey cont. CCN spec [m-3 s-1]
    ...
    function newNi = nucl(T0)
        if (T0 > 237 && T0 < 273)
            ...
            lapse = -6/1000;
                % moist adiabatic lapse rate of 6 K/km -> [K m-1]
            newNi = aa*uz*lapse*exp(-0.125*(T0 - 273.2))/factor;
        else 
            newNi = 0;
        end
    end
