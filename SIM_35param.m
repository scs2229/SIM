% SIM_35param is a zero-dimensional model which calculates ice and
...
function [Nice,cont,INPt,tauNUC] = SIM_35param(etabr,etaagg,etaRSg,etaRSG,...
    ...
    function p = pShatter(T)
        if (T < 266)
            mu  = 258; sigma = 5;
            max = pmax; % normalize to a max probability of 20%
    ...
    function pf = pFreeze(T, D, ni)
       if (Nd ~= 0 && D > 100*10^(-6) && T < 273 && ni < INPstop)
           f    = 1;
